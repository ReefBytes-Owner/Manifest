...
Feeds the identity-based debt ratchet (Phase 3 chunk C3, packaged separately
from this skill): the ratchet's identity never includes a line number (line
numbers churn on unrelated edits), so it needs something else to scope a
finding to the right function/class rather than "the whole file". This
...
def _bash_anchor(lines: list[str], line: int) -> str:
    ...
    Not a full parser -- Bash function nesting is rare in the scripts this
    checker scans. A function is considered closed at the first line that is
